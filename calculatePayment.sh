...
if [[  -z ${arr_of_params[0]} ||  -z ${arr_of_params[-1]}]]; then
    empty_params=0;
    for((i = 0; i < 2; i++)); do
...
elif  [[ ! ${arr_of_params[-1]} =~ $regex_numbers ]]; then
...
if  [[ $error -eq 1 ]]; then
    ...
    if [[ ${arr_of_params[-1]} -lt $sum ]]; then
        printf "You need to add %.2f shekel to pay the bill\n" "$(($sum - ${arr_of_params[-1]}))"
    elif [[ ${arr_of_params[-1]} -gt $sum ]]; then
        printf "Your change is %.2f shekel\n" "$((${arr_of_params[-1]} - $sum))"
    elif [[ ${arr_of_params[-1]} -eq $sum ]]; then
        echo "Exact payment"
    fi
fi
